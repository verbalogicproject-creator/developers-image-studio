...
import { bufferToFloat32, float32ToBuffer } from "./vector_math";
...
export class MemoryDatabase {
  ...
  private initSchema() {
    ...
    try {
      ...
      console.warn("FTS5 table initialization skipped:", err);
      this.hasFTS5 = false;
    }
  }
  ...
  // ==========================================
  // EXPERIENTIAL & MULTIMODAL MEMORIES
  // ==========================================
  ...
  upsertMemory(memory: MemoryRecord) {
    ...
    const stmt = this.db.prepare(`
      INSERT OR REPLACE INTO memories (
        id, memory_type, modality, title, content, metadata,
        embedding, embedding_model, embedding_dimension, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
  ...
  getAllMemoriesWithEmbeddings(): MemoryRecord[] {
    const stmt = this.db.prepare(`
      SELECT id, memory_type as memoryType, modality, title, content,
  ...
  // ==========================================
  // RELATIONS (GRAPHRAG ARCHITECTURE GRAPH)
  // ==========================================
  ...
  getStats(): IndexStats {
    const filesCount = (
      this.db.prepare("SELECT COUNT(*) as count FROM files").get() as any
    )?.count || 0;
    const chunksCount = (
      this.db.prepare("SELECT COUNT(*) as count FROM chunks").get() as any
    )?.count || 0;
    const memoriesCount = (
      this.db.prepare("SELECT COUNT(*) as count FROM memories").get() as any
    )?.count || 0;
    const relationsCount = (
      this.db.prepare("SELECT COUNT(*) as count FROM relations").get() as any
    )?.count || 0;
    const lastIndexedAt = (
      this.db.prepare("SELECT MAX(indexed_at) as max FROM files").get() as any
    )?.max || 0;
